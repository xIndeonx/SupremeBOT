...
customCommands = function () {
	constants.client.on('message', function (message) {
		...
		try {
			if (command.startsWith('1=0')) {
			...
			else if (command.startsWith('australia')) {
				...
				else if (args[0] === '3') return message.channel.send('<@' + constants.LUCAS_ID + '>, ab id Duschi');
			...
			message.channel.send({
				embed: {
					title: 'Error',
					color: constants.red,
					description: `Fatal error while executing the \`${command}\` command.`,
				},
			});
			return;
		}
